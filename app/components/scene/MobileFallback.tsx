...
import { Frown, LoaderCircle, X } from 'lucide-react';

export function MobileFallback({ isMobile }: { isMobile: boolean | null }) {
    if (isMobile === null) {
        return (
        ...
        return (
            <div className="flex flex-col gap-6 h-screen items-center justify-center w-full">
                ...
                <div className="fixed flex items-center bottom-4 text-xs text-gray text-center">
                    <span>Размер экрана: {typeof window !== 'undefined' ? window.innerWidth : '?'}</span>

                    <X size={12} className="flex items-center justify-center" />

                    <span>{typeof window !== 'undefined' ? window.innerHeight : '?'}px</span>
                </div>
            </div>
        );
    }
